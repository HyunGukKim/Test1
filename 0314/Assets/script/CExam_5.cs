...
public class CExam_5 : MonoBehaviour {
    ...
    delegate int KhgCallFunc(int tVal);

	// Use this for initialization
	void Start () {
        //람다표현식 
        KhgCallFunc tRyuCall = tVal => { return tVal * tVal; };
        Debug.Log(tRyuCall(3));

        //매개변수가 단 하나이고, 그 형식을 추론할 수 있는 경우에는 중괄호쌍을 생략하고 더 간단히 표기할 수도 있다.
        //RyuCallFunc tRyuCall = tVal=>tVal*tVal;

        //Func<매개변수,리턴값>
        //Func, Action대리자와 함께 쓰이는 예시.
        //( Func, Action대리자는 C#에서 제공하는 범용 대리자이다. )
        Func<int, int> tRyuFunc = tVal => tVal * tVal;
        Debug.Log(tRyuFunc(4));


        //외부변수 갈무리
        //outer variable, captured variable, closure
        int tFactor = 777;//outer variable
        Func<int, int> tRyuFuncWithFactor = tVal => tVal * tFactor; //captured variable, closure
        Debug.Log(tRyuFuncWithFactor(4));
